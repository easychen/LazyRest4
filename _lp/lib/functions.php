...
{
    $dbname = '_LP4_DB_' . md5($dsn);

    if( !isset( $GLOBALS[$dbname ] ) )
        $GLOBALS[$dbname ] = new Database();

    return  $GLOBALS[$dbname];
}

function mydb( $dsn , $user , $password )
{
       $dbname = '_LP4_MYDB_' . md5($dsn);

    if( !isset( $GLOBALS[$dbname ] ) )
        $GLOBALS[$dbname ] = new Database( $dsn , $user , $password );
